...
create table dbo.x(a char(2) not null, b char(2) not null, placeholder char(53) not null, c varchar(5) not null, constraint pk_x primary key(b)
);

go
--Insert some rows
insert into dbo.x(a, b, placeholder, c ) 
select right('0' + convert(varchar(2),number), 2), right('0' + convert(varchar(2),number), 2), 'x', 'ab-cd'
